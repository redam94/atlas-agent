...
class ProjectUpdate(AtlasRequestModel):
    ...
    Fields that map to NOT NULL columns in the DB cannot be explicitly set
    to ``null`` — that would produce a 500 at flush time. The
    ``_reject_explicit_null_on_required_fields`` validator catches this at
    the API boundary so the response is a 422 instead.
    """

    name: str | None = Field(default=None, min_length=1, max_length=200)
    description: str | None = Field(default=None, max_length=2000)  # nullable in DB
    ...
    @classmethod
    def _reject_explicit_null_on_required_fields(cls, data):
